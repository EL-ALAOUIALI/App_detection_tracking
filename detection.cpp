    ...
    ui(new Ui::detection)
{
    ...
    connect(ui->executehog, SIGNAL (released()), this, SLOT (executehog()));
}
...
// draw one HOG over one cell
 void detection::drawHOG(std::vector<float>::const_iterator hog, // iterator to the HOG
...
// Draw HOG over an image
 void detection::drawHOGDescriptors(const cv::Mat &image,  // the input image
...
//
    ...
    for (unsigned j = 1; j < positives.size(); j++) {
    ...
    for (unsigned j = 0; j < negatives.size(); j++) {
    ...
    for (unsigned i = 0; i < peoples.size(); i++)
...
void detection::executehog()
{
    ui->executehog->setDisabled(true);
    HOG();
}
